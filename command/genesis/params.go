...
func (p *genesisParams) validateFlags() error {
	...
	// Extract native token metadata for both PolyBFT and IBFT (needed for staking contract)
...
func (p *genesisParams) predeployStakingSC() (*chain.GenesisAccount, error) {
	// Safety check for nativeTokenConfig
	if p.nativeTokenConfig == nil {
		return nil, fmt.Errorf("native token config is not initialized")
	}
	if p.nativeTokenConfig.Owner == types.ZeroAddress {
		return nil, fmt.Errorf("native token owner is not set (zero address)")
	}
	...
	stakingAccount, predeployErr := stakingHelper.PredeployStakingSC(
		...
		stakingHelper.PredeployParams{
			...
			OwnerAddress:      p.nativeTokenConfig.Owner.String(),
		})
	if predeployErr != nil {
